...
import { applicantNavItems } from "@/data/constants"

export default function ApplicantNavigation({ applicantId, jobId }: { applicantId: string, jobId: string }) {
        ...
        <div className="flex flex-col lg:flex-row justify-between px-4 lg:px-8 pt-4 pb-4 lg:pb-0 bg-gray-100">
            <nav className="w-full lg:w-auto">
                <ul className="flex flex-col lg:flex-row gap-2">
                    {applicantNavItems.map((item, idx) => (
                        <Link 
                            key={idx} 
                            href={{
                                pathname: `/dashboard/applicant/${applicantId}/${item.path}`,
                                query: { jobId }
                            }}
                        >
