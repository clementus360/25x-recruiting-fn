...
import { getApplicantsForJob } from "@/data/jobsData";
import { useParams } from "next/navigation";
import { useError } from "@/context/ErrorContext";
import { ApplicantRow } from "@/components/Dashboard/Jobs/Job/Applicants/ApplicantRow";
...
export default function Applicants() {
  ...
  const fetchApplicants = async () => {
    ...
    try {
      ...
      const data = await getApplicantsForJob(jobId, token, currentPage, filters);

      setApplicants(data.Applicants);
      setTotalPages(data.pageCount);
      setResults(data.totalApplicants)
    } catch (error: any) {
      setError(error.message ? error.message : `An error occured while loading applicants`);
    } finally {
  ...
  return (
    <section className="relative flex flex-col gap-4">
      ...
      {isAddApplicantOverlayOpen && <AddApplicantOverlay handleLoad={handleLoadData} onClose={handleCloseAddApplicantOverlay} />}
